...
const CommentModal = ({datasTweet, userState}) => {
    ...
    // mise a jour des nouveaux commentaire de tweets 
    useEffect(() => {
        if (!socket){
        ...
        return () => socket.off(`comment_${datasTweet._id}`);
    }, [socket])
                    ...
                    <div className="overflow-y-auto h-80">
                        {
                            !loading ? (
                                <>
                                    {dataCommentaire?.donneEnvoye && dataCommentaire?.donneEnvoye?.sort((a, b) => new Date(b?.createdAt) - new Date(a?.createdAt)).map((element, id) => (
